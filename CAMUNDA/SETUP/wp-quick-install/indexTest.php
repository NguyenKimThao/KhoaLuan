...
if (isset($_GET['action'])) {
    ...
    switch ($_GET['action']) {
        ...
        case "check_before_upload" :
            ...
            try {
                $conn = new PDO("mysql:host=" . $_POST['dbhost'], $_POST['uname'], $_POST['pwd']);
...
//            $version = "4.9.8";
            ...
            if (!file_exists(WPQI_CACHE_CORE_PATH . 'wordpress-' . $version . '-' . $language . '.zip')) {
                file_put_contents(WPQI_CACHE_CORE_PATH . 'wordpress-' . $version . '-' . $language . '.zip', file_get_contents($wp->download));
            }
...
//            $version = "4.9.8";
            ...
            if ($zip->open(WPQI_CACHE_CORE_PATH . 'wordpress-' . $version . '-' . $language . '.zip') === true) {
        ...
        case "success" :
            ...
            echo '<div id="errors" class="alert alert-danger"><p style="margin:0;"><strong>' . _('Warning') . '</strong>: You have created wordpress name is ' . $_POST['wordpressName'] . ' .</p></div>';
            $server_camunda = 'http://localhost:8080/camunda-extend/#/workspace/' . $_POST['workspaceName'] . '/wordpress/' . $_POST['wordpressName'];
            // Link to the admin
...
//            echo '<a href="' . home_url() . '" class="button" target="_blank">' . _('Go to website') . '</a>';
            echo '<a href="' . $server_camunda . '" class="button">' . _('Continue') . '</a>';

            break;
    }
} else {
    if (!isset($_GET['workspaceName']))
        exit();
    else
        $workspacename = $_GET['workspaceName'] . time();
    ?>
            ...
            if (is_writable($parent_dir)) {
                ...
                <form method="post" action="">
                    ...
                    <table class="form-table hidden">
                        <tr>
                            <th scope="row"><label for="workspaceName"><?php echo _('Wordspace name'); ?></label></th>
                            <td><input name="workspaceName" id="workspaceName" type="text" size="25" value="<?php echo $_GET['workspaceName'] ?>" class="required" /></td>
                        </tr>
                        <tr>
                            <th scope="row"><label for="wordpressName"><?php echo _('wordpressName name'); ?></label></th>
                            <td><input name="wordpressName" id="wordpressName" type="text" size="25" value="<?php echo$workspacename ?>" class="required" /></td>
                        </tr>
                        <tr>
                            <th scope="row"><label for="dbname"><?php echo _('Database name'); ?></label></th>
                            <td><input name="dbname" id="dbname" type="text" size="25" value="wordpress<?php echo$workspacename ?>" class="required" /></td>
                    ...
                    <table class="form-table">
                        ...
                        <tr>
                            <th scope="row">
                                <label for="localhost_wordpress"><?php echo _('Host Wordpress'); ?></label>
                            </th>
                            <td>
                                <input name="localhost_wordpress" type="text" id="localhost_wordpress" size="25" value="http://localhost:8888/wp-quick-install/" />
                            </td>
                        </tr>
                        <tr class="hidden">
                            <th scope="row">
                            ...
                            <td>
                                <input name="directory" type="text" id="directory" size="25" value="<?php echo$workspacename ?>" />
                            </td>
                        </tr>
                        <tr class="hidden">
                            <th scope="row"><label for="weblog_title"><?php echo _('Site Title'); ?></label></th>
                            <td><input name="weblog_title" type="text" id="weblog_title" size="25" value="<?php echo$workspacename ?>" class="required" /></td>
                        </tr>
                        <tr>
                ...
                <script src="assets/js/scriptcamunda.js"></script>
            <?php } else {
                ?>
