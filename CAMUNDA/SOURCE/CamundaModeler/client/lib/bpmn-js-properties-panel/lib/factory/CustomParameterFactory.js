...
var DELETE_ROW_BUTTON_SNIPPET = '<button class="clear" style="float: right" data-action="deleteElement">' +
    '<span>X</span>' +
    '</button>';

function createInputRowTemplate(properties, canRemove, fields, value) {
    var template = TABLE_ROW_DIV_SNIPPET;
    template += createInputTemplate(properties, canRemove, fields, value);
...
function createInputTemplate(properties, canRemove, fields, value) {
    var columns = properties.length;
    var template = '';
    var htmlOptions = '';
    for (var i = 0; i < fields.length; i++) {
        var val = '';
        if (fields[i].name == value[properties[0]]) {
            val = 'selected';
        }
        htmlOptions += '<option value="' + fields[i].name + '"' + val + ' disable>' + fields[i].name + '</option>';
    }
    for (var i = 0; i < properties.length; i++) {
        var prop = properties[i];
        template += '<select style="width:45%; height: 23px; float: left" class="bpp-table-row-columns-' + columns + ' ' +
            (canRemove ? 'bpp-table-row-removable' : '') + '" ' +
            'id="camunda-table-row-cell-input-value-' + i + '" ' +
            'type="text" ' +
            'name="' + prop + '" >' + htmlOptions + '</select>';

        i++;

        prop = properties[i];
        template += '<input style="width:45%; height: 23px" class="bpp-table-row-columns-' + columns + ' ' +
            (canRemove ? 'bpp-table-row-removable' : '') + '" ' +
...
module.exports = function (options) {
    ...
    var factory = {
        ...
        html: (canAdd ?
            ...
            '<button class="add" data-action="addElement1"><span>+</span></button>' +
            '</div>' : '') +
        ...
        get: function (element, node) {
            ...
            for (var i = 0; i < boElements.length; i++) {
                var selectTeg = $('#camunda-table-row-cell-input-value-' + i);
                selectTeg.value = boElements[i].field;
            }
        ...
        set: function (element, values, node) {
            ...
            else if (action.id === 'add-element1') {
        ...
        createListEntryTemplate: function (value, index, selectBox) {
            return createInputRowTemplate(modelProperties, canRemove, fields, value);
        },

        addElement1: function (element, node, event, scopeNode) {
            var template = domify(createInputRowTemplate(modelProperties, canRemove, fields, ''));
