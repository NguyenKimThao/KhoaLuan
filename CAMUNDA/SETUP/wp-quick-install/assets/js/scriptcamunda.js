$(document).ready(function () {
    ...
    function  GetUrl()
    {
        var localhost = $('#localhost_wordpress').val();
        if (localhost[localhost.length - 1] === '/')
            localhost = localhost.substring(0, localhost.length - 1);
        return localhost + "/index.php";
    }
    ...
    $('#submit').click(function () {
        ...
        if (!errors) {
            ...
            $.post(GetUrl() + '?action=check_before_upload', $('form').serialize(), function (data) {
                ...
                if (!errors) {
                    $('form').fadeOut('fast', function () {
                        ...
                        $.post(GetUrl() + '?action=download_wp', $('form').serialize(), function () {
                            unzip_wp();
                        });
                    });
                } else {
    ...
    function unzip_wp() {
        ...
        $.post(GetUrl() + '?action=unzip_wp', $('form').serialize(), function (data) {
            wp_config();
        });
    }
    ...
    function wp_config() {
        ...
        $.post(GetUrl() + '?action=wp_config', $('form').serialize(), function (data) {
            install_wp();
        });
    }
    ...
    function install_wp() {
        ...
        $.post(GetUrl() + '/wp-admin/install.php?action=install_wp', $('form').serialize(), function (data) {
            install_theme();
    ...
    function install_theme() {
        ...
        $.post(GetUrl() + '/wp-admin/install.php?action=install_theme', $('form').serialize(), function (data) {
            install_plugins();
        });
    }
    ...
    function install_plugins() {
        ...
        $.post(GetUrl() + '?action=install_plugins', $('form').serialize(), function (data) {
            $response.html(data);
            success();
        });
    }
    ...
    function success() {
        ...
        $.post(GetUrl() + '?action=success', $('form').serialize(), function (data) {
